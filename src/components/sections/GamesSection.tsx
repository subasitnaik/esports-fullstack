...
function GameCard({
  ...
  game: { name: string; slug: string; image?: string };
}) {
  const [imgError, setImgError] = useState(false);
  const [srcIndex, setSrcIndex] = useState(0);
  const basePath = game.image ?? `/images/games/${game.slug}`;
  const src = basePath + IMAGE_EXTS[srcIndex];
      ...
      >
        {!imgError ? (
          <Image
            ...
            unoptimized
            onError={() => {
              if (srcIndex < IMAGE_EXTS.length - 1) {
                setSrcIndex((i) => i + 1);
              } else {
                setImgError(true);
              }
...
export function GamesSection() {
  ...
  const updateActiveIndex = useCallback(() => {
    ...
    setActiveIndex(Math.min(Math.max(0, index), games.length - 1));
  }, [games.length]);
